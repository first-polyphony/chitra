...
- **`chitra.rate_limit_guard`** (`chitra-rate-limit-guard`, `default_enabled: false`) — a one-shot sweep composing `chitra.usage` + `chitra.goals` + the dispatch queue into the graceful pause/resume lifecycle: a fresh `pause` verdict for a tracked lane enqueues a fixed checkpoint nudge and holds its goal record (`rate-limit:<window>`, preserving the goal as the re-arm payload); a due hold auto-resumes once a fresh verdict confirms `ok` again, enqueueing a re-arm nudge built only from the lane's own stored `goal`/`done_when`. Run it under an external timer (systemd timer / cron) once per host — it is not a daemon and adds none. `chitra.dispatchd` reads the same goal record to freeze ordinary new-work delivery to a rate-limit-held session until the hold clears.
...
`dispatchd` also accepts `--policy-config-path`, `--invalid-orders-dir`, `--capture-lines`, `--post-paste-wait-seconds`, `--transcript-recency-seconds`, `--lane-lock-timeout-seconds`, and `--goals-root` (the `chitra.goals` store consulted for the rate-limit freeze check; default: `CHITRA_STATE_DIR`); see `dispatchd --help`. The generic replay evaluator and fixture workflow are documented in [`docs/self-tuning.md`](docs/self-tuning.md).
