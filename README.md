...
- **`chitra.goal_enforcement`** — immutable goal/candidate/review contracts and a deterministic, fail-closed release gate. It hashes an exact strategic goal version and candidate, requires at least two unique non-author reviews, and releases only unanimous, correctly bound acceptances. It does not draft, review, or call a model.

The top-level `adapter_prototype.goal_loop` prototype is deliberately excluded from the wheel. It owns the model-backed writer/reviewer/fixer loop and consumes `chitra.goal_enforcement`; this keeps all reasoning on the deployment-adapter side of the deterministic-core boundary.
